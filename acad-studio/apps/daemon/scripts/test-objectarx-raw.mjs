...
assert(mepbridge.includes("mepRawRegisterCommands"), "mepbridge registers raw commands");
...
const hasBundle =
  existsSync(join(root, "build/Acad-Bridge.bundle")) ||
  existsSync(join(root, "build/MEP-Bridge.bundle"));
if (existsSync(join(root, "build"))) {
  assert(hasBundle, "plugin bundle present if build/ exists (Acad-Bridge or legacy MEP-Bridge)");
} else {
  assert(true, "no build/ dir yet (SDK may be absent) — source/build.sh still deliverable");
}

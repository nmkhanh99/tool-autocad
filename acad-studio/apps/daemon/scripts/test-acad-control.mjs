 ...
 * Run: cd acad-studio/apps/daemon && npx tsx scripts/test-acad-control.mjs
 */
...
assert(
  appload.endsWith("Acad-Bridge.bundle") ||
    appload.includes("Acad-Bridge.bundle") ||
    appload.includes("MEP-Bridge.bundle"),
  "appload is outer package bundle (Acad-Bridge primary, MEP-Bridge legacy ok)",
);
assert(appload.includes("ApplicationPlugins"), "appload under ApplicationPlugins");
assert(!appload.includes("AcadBridge.bundle/Contents"), "appload not nested module path");
assert(typeof ctrl.isPluginInstalled === "function", "isPluginInstalled exported");
assert(ctrl.PLUGIN_BINARY_REL === "Contents/MacOS/AcadBridge", "flat binary relative path");

const scratch = ctrl.scratchDwgPath("/Users/khanhnm/Desktop/tool-autocad");
assert(scratch.endsWith("ACAD-RAW-scratch.dwg"), "scratch DWG path");
assert(scratch.includes("acad-studio/.work"), "scratch under acad-studio/.work");
...
const staticChecks = ctrl.buildStaticHealthChecks({
  ...
  bridgeOk: true,
  bridgeDetail: "/Users/x/Acad-Bridge/results",
  pluginOk: true,
  sdkOk: true,
  clangOk: true,
  agentsOk: true,

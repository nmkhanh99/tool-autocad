...
/** Resolve at call time so ACAD_BRIDGE_DIR / live plugin dir can change. */
function liveBridgeDir(): string {
  return resolveBridgeDir();
}
function liveRawJob(): string {
  return rawJobPath(liveBridgeDir());
}
function liveRawDone(): string {
  return rawDonePath(liveBridgeDir());
}

function ensureBridge(): void {
  ensureBridgeLayout(liveBridgeDir());
}

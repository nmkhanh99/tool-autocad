...
## Backlog tích hợp vào MEP Studio (cập nhật)

| # | Tính năng | Effort | Trạng thái | Mô tả |
|---|---|---|---|---|
| 1 | **Realtime Events Bridge (push thay polling)** ✅ ĐÃ TÍCH HỢP | med | buildable-now | AutoCAD tự báo cho MEP Studio mọi sự kiện: mở/đóng/đổi bản vẽ active, lệnh vừa kết thúc (MOVE/COPY/E |
...
| 5 | **Live BOQ — dirty-tracking incremental** ✅ ĐÃ TÍCH HỢP | med | buildable-now | Bảng khối lượng trong MEP Studio tự cập nhật live khi user vẽ/sửa/xóa, không cần bấm nút quét lại. |
...
| 8 | **Doc Control — panel dự án điều khiển documents**  | low | buildable-now | MEP Studio liệt kê mọi DWG đang mở (đường dẫn, trạng thái active), mở file dự án, chuyển tab, đóng f |
...
| 18 | **Batch xuất PDF hồ sơ**  | med | needs-restart-test | Nút 'Xuất PDF toàn bộ layout' trong MEP Studio: plot hàng loạt layout khung tên ra PDF không cần bấm |
...
- OPM Property Palette (dynprops.h IPropertyManager/IDynamicProperty, category.h, opmdialog.h) — bọc #ifdef _ADESK_WINDOWS_ + thuần COM; không thêm được property MEP vào palette Properties trên Mac — hiển thị trong panel riêng của MEP Studio

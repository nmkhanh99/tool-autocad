#!/usr/bin/env node
/**
 * MEP Studio — 1 entrypoint (giống `pnpm tools-dev` của Open Design).
...
console.log(`\n  🧰 MEP Studio — khởi động (${MODE})\n`);
run("daemon", ["--filter", "@mep/daemon", "dev"], "36");
run("web", ["--filter", "@mep/web", "dev"], "33");
...
if (MODE === "web") {
  ...
  console.log(`\n  🖥  Mở cửa sổ app...\n`);
  run("electron", ["--filter", "@mep/desktop", "dev"], "35");
}

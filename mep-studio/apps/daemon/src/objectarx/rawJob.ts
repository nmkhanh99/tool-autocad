 ...
 * Format: TAB-separated lines written to ~/MEP-Bridge/raw.job
 * Result: JSON in ~/MEP-Bridge/raw.done
 *
...
export function defaultParamsFor(id: string): RawParams {
  switch (id) {
    ...
    case "db.text_mtext":
      return { kind: "text", x: 0, y: 0, h: 250, text: "MEP RAW", layer: "0" };
    case "db.block":
      return { action: "define", name: "MEP_RAW_BLK", x: 0, y: 0 };
    case "db.layer":
      return { name: "MEP-RAW-TEST", aci: 3 };
    case "db.xdata":
      return { action: "probe" };
    case "db.handle":
      return { action: "probe" };
    ...
    case "ed.sysvar":
      return { name: "CLAYER" };
    case "ed.printf":
      return { msg: "[MEP RAW] acutPrintf probe" };
    case "db.units":
      return { action: "get" };
    case "db.extents":
      return { action: "model" };
    case "db.group":
      return { action: "probe", name: "MEP_RAW_GRP" };
    case "db.nod":
      return { action: "probe", key: "MEP_STUDIO" };
    case "db.transaction":
      return { action: "probe" };
    case "db.hatch":

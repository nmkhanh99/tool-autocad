...
export default function Page() {
  ...
  useEffect(() => { loadAgents(); loadConvs(); loadRawCatalog(); }, []);
    ...
    try {
      ...
      patchLast((m) => {
        m.fnResult = {
          live: true,
          hint: r.hint || (r.path
            ? `✓ Đã mở: ${r.path}. Đợi plugin MepBridge nạp (~5–15s), rồi chạy ObjectARX raw.`
            : "✓ Đã mở AutoCAD. Trong CAD gõ NEW (hoặc File → New), đợi plugin, rồi chạy raw."),
        };
      });
  ...
  async function runFix(action: string) {
    ...
    try {
      ...
      setMessages((p) => [...p, {
        ...
        fnResult: {
          live: true,
          hint: r.detail || r.hint || (r.ok ? "OK" : (r.error || "Lỗi")),
          raw: r,
        },
  ...
  /** Demo: stage on CAD as PREVIEW only — user must click Chấp nhận / Không chấp nhận. */
  async function demoDraw() {
    const fn = byId("drawpipes")!;
    const pipes = [
      { system: "thoatxi", dn: 150, points: [[0, 0], [20000, 0], [20000, 15000]] },
      { system: "thonghoi", dn: 100, points: [[0, 5000], [18000, 5000]] },
    ];
    await runPreview(fn, "", { pipes });
  }
  ...
  async function runLive(fn: Fn, params: any, target?: string) {
    ...
    const { alive, docs } = await loadDocs();
    if (!alive) return patchLast((m) => { m.error = "Plugin MepBridge không phản hồi — khởi động lại AutoCAD (plugin tự nạp) rồi thử lại."; });
  ...
  return (
    <div className="app">
      <aside className="side">
        <div className="brand">MEP<span>·</span>Studio</div>
      ...
      <main className="main">
        <div className="topbar">
          ...
          <button className="pillbtn" onClick={openHealth}>⚙ Kiểm tra AutoCAD</button>
          {acadLive && (
        ...
        <div className="chat" ref={chatRef}>
          <div className="wrap">
            {messages.length === 0 && (
              <div className="empty"><h2>Trợ lý MEP cho AutoCAD</h2>
                <p>Bấm <b>🧰 Chức năng</b> để chạy nhanh, hoặc gõ yêu cầu — agent tự đề xuất & thực hiện.</p></div>
            )}
        ...
        <footer>
          <div className="composer">
            <textarea value={input} placeholder="Nhắn cho trợ lý MEP…  (Enter gửi)"
      ...
      {panel && (
        <aside className="fnpanel">
          ...
          <div className="demorow" onClick={() => openAutoCAD(undefined, { newFile: true })}
            title="Mở AutoCAD 2027 + tạo/mở file scratch mới (MEP-RAW-scratch.dwg) để thao tác ObjectARX raw">
            🚀 Mở AutoCAD + file mới
          </div>
          <div className="demorow" onClick={demoDraw} title="Preview ống mẫu trên AutoCAD — phải bấm Chấp nhận/Không chấp nhận">
            🧪 Vẽ thử (preview) — xem CAD rồi bấm nút
          </div>
      ...
      {healthOpen && (
        <div className="modal" onClick={() => setHealthOpen(false)}>
          <div className="modalbox" onClick={(e) => e.stopPropagation()} style={{ width: 560 }}>
            <h3>⚙ Kiểm tra & cấu hình AutoCAD</h3>
                ...
                {health.checks.map((c: any) => (
                  <div key={c.id} className="hrow">
                    ...
                    <div className="hbody">
                      ...
                      {c.appload && !c.ok && c.id === "pluginlive" && (
  ...
  return (
    <div className="msg function">
      ...
      <div className="bubble">
        ...
        {r && r.live && (
          <>
            <div className="livebox">{r.hint || `✓ Đã gửi — AutoCAD tự thực hiện (plugin MepBridge).`}</div>
            {r.selRows && (
              <>
